...
library SnowLib {
    ...
    // }
    function _perc_of_uint256(uint16 _perc, uint256 _num) public pure returns (uint256) {
        require(_perc <= 10000, 'err: invalid percent');
        // return _perc_of_uint64_unchecked(_perc, _num);
        return (_num * uint256(_perc * 100)) / 1000000; // chatGPT equation
    }
    ...
    //     return string(last4);
    // }
    function _addBytes32ToArraySafe(bytes32 _bytes, bytes32[] memory _arr, bool _safe) private pure returns (bytes32[] memory) {
        if (_bytes == 0) { return _arr; }

        // safe = remove first (no duplicates)
        if (_safe) { _arr = _remBytes32FromArray(_bytes, _arr); }

        // perform add to memory array type w/ static size
        bytes32[] memory _ret = new bytes32[](_arr.length+1);
        for (uint i=0; i < _arr.length;) { _ret[i] = _arr[i]; unchecked {i++;}}
        _ret[_ret.length-1] = _bytes;
        return _ret;
    }
    function _remBytes32FromArray(bytes32 _bytes, bytes32[] memory _arr) private pure returns (bytes32[] memory) {
        if (_bytes == 0 || _arr.length == 0) { return _arr; }
