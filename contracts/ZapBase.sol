...
contract ZapBase is MultipleOperator, ReentrancyGuard {
    ...
    address devAddress; // house: now set in child class (ZapV3.CONF_setConfig)
    ...
    function setCoreValues(
        ...
        // devAddress = msg.sender; // house: now set in child class (ZapV3.CONF_setConfig)
    }

    function getCurrentCoreValues() external view returns (address[] memory) {
        address[] memory currentValues = new address[](6);
        currentValues[0] = routerAddress;
        currentValues[1] = PancakeSwapFactory;
        currentValues[2] = masterchef;
        currentValues[3] = mainToken;
        currentValues[4] = mainTokenLP;
        currentValues[5] = WETH;
    ...
    function processFee(
        ...
        if (tokenType[_inputToken] == TokenType.LP) {
    ...
    function _zapIntoTokenTaxWrapper(
        ...
        if (
            ...
            // uint256 tokenTaxRate = SnowToken(_inputToken).getCurrentTaxRate(); // legacy instance
            uint256 tokenTaxRate = ISnowTokenX(_inputToken).getCurrentTaxRate();
    ...
    */
    ...
    function _safeTransferFrom(
        ...
        IERC20(_inputToken).safeTransferFrom(_from, address(this), _amount);
    }
    ...
    */
    ...
    function withdraw(address token, bool isNative) external onlyOperator {
        if (isNative) {
    ...
    // house_011525: dev address handle in SnowConfig
